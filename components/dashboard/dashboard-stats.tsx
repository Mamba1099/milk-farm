...
export const TreatmentStatsCard: React.FC = () => {
  const { treatments } = useDashboardStats();

  if (treatments.isLoading) {
    return (
      <LoadingCard
        title="Health & Treatments"
        icon={<Icons.heartPulse className="h-5 w-5 text-red-800" />}
        gradient="bg-gradient-to-br from-red-100 to-red-200 border-red-300"
      />
    );
  }
  ...
  if (treatments.isError) {
    return (
      <ErrorCard
        ...
        icon={<Icons.heartPulse className="h-5 w-5 text-red-800" />}
        gradient="bg-gradient-to-br from-red-100 to-red-200 border-red-300"
        error="Failed to load treatment data"
      />
    );
  }

  if (!treatments.data || treatments.data.totalRecords === 0) {
    return (
      <NoDataCard
        title="Health & Treatments"
        icon={<Icons.heartPulse className="h-5 w-5 text-red-800" />}
        gradient="bg-gradient-to-br from-red-100 to-red-200 border-red-300"
        message="No treatment records found. Great! Your animals are healthy!"
      />
    );
  }

  const stats = treatments.data;
      ...
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-4">
          <div className="text-center p-3 bg-white/60 rounded-lg">
            ...
            <div className="text-xs text-red-600">Total Treatments</div>
          </div>
          <div className="text-center p-3 bg-white/60 rounded-lg">
            <div className="text-2xl font-bold text-red-800">
              {stats.thisMonth}
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-between items-center">
            <span className="text-sm text-red-600">Last Treatment:</span>
            <span className="text-sm font-medium text-red-800">
              {stats.lastTreatmentDate
                ? new Date(stats.lastTreatmentDate).toLocaleDateString()
                : "No treatments"}
            </span>
          </div>
        </div>
        <div className="mt-4 p-3 bg-white/40 rounded-lg">
          <div className="text-center">
            <div className="text-sm text-red-700">
              Health Status:{" "}
              <span className="font-semibold text-green-600">Monitoring</span>
            </div>
          </div>
        </div>
      </div>
