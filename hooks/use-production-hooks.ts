...
// Serving hooks
export const useServings = (params?: {
  page?: number;
  limit?: number;
  outcome?: string;
  search?: string;
}) => {
  return useQuery<ServingResponse>({
    queryKey: ["servings", params],
    queryFn: async () => {
      ...
      const response = await apiClient.get(`/servings?${searchParams}`);
      return response.data;
    },
...
export const useCreateServing = () => {
  const queryClient = useQueryClient();

  return useMutation<{ serving: ServingRecord }, Error, CreateServingData>({
    mutationFn: async (data: CreateServingData) => {
...
export const useUpdateServing = () => {
  ...
  return useMutation<
    ...
    Error,
    { id: string } & Partial<CreateServingData>
  >({
    mutationFn: async ({ id, ...data }) => {
      const response = await apiClient.put(`/servings/${id}`, data);
      return response.data;
    },
    onSuccess: () => {
      // Invalidate serving queries
