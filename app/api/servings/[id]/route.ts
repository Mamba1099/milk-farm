...
async function handleUpdateServing(
  request: NextRequest,
  { params }: { params: { id: string } }
) {
  try {
    ...
    // Check if serving record exists
    const existingServing = await prisma.serving.findUnique({
      where: { id },
    });
    ...
    const updatedServing = await prisma.serving.update({
      ...
      data: {
        ...
        ...(actualBirthDate && { actualBirthDate: new Date(actualBirthDate) }),
    ...
    return NextResponse.json({ serving: updatedServing }, { status: 200 });
  } catch (error) {
...
export const PUT = withApiTimeout(handleUpdateServing, 30000);
